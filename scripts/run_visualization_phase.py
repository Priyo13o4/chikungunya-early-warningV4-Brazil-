...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--payload-csv",
        type=Path,
        default=PROJECT_ROOT / "outputs" / "reports" / "visualization_payload.csv",
        help="Path to visualization payload CSV",
    )
    parser.add_argument(
        "--payload-metadata",
        type=Path,
        default=PROJECT_ROOT / "outputs" / "reports" / "visualization_payload_metadata.json",
...
def main() -> None:
    ...
    payload_csv_path = _resolve_from_project_root(args.payload_csv)
    payload_metadata_path = _resolve_from_project_root(args.payload_metadata)
    output_dir = _resolve_from_project_root(args.output_dir)
